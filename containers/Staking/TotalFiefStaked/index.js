...
const TotalFiefStaked = () => {
  const classes = useStyles();
  const {
    stakingInfo: { totalTokenStaked = 0, percChangedStaked = 0 },
  } = useStaking();

  return (
    <BorderCardWrapper className={classes.card} rootClassName={classes.root}>
      <Typography className={classes.title}>Total Magic Blocks Staked</Typography>
      <PercentLabel value={percChangedStaked} />

      <Typography color='textSecondary' className={classes.balance}>
        {formatNumber(totalTokenStaked)} <span>MABS</span>
      </Typography>
    </BorderCardWrapper>
  );
};

export default memo(TotalFiefStaked);
